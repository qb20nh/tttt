...
export const runBenchmark = (options: BenchmarkOptions) => {
    ...
    if (hasTimeout === hasSearchDepth) {
        throw new Error("Benchmark Error: Exactly one of 'timeout' or 'searchDepth' must be provided.");
    }
    ...
    if (hasTimeout) console.log(`Mode: Time Limit (${timeMs}ms). Max Search Depth: ${targetDepth}`);
