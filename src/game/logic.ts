...
export const generateBoard = (currentDepth: number): BoardNode => {
  ...
  return {
    ...
    children: Array(9)
      .fill(null)
      .map(() => generateBoard(currentDepth - 1)),
  }
}
...
export const isFull = (board: BoardNode): boolean => {
  ...
  return board.children.every(
    (child) => child.winner !== null || child.value !== null || isFull(child)
  )
}
...
export const canWin = (board: BoardNode, player: Player): boolean => {
  ...
  for (const pattern of WIN_PATTERNS) {
    const [a, b, c] = pattern
    if (
      canWin(board.children[a], player) &&
...
export const getPathFromCoordinates = (
  gridX: number,
  gridY: number,
  depth: number
): number[] => {
...
export const getNextConstraint = (
  path: number[],
  wonDepth: number
): number[] => {
