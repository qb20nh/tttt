...
export const useGameState = (
  ...
  const applyMove = useCallback(
    (result: { newBoard: BoardNode; rootWinner: Winner; nextC: number[] }) => {
      ...
      setActiveConstraint((prev) => {
        ...
        if (
          prev.length === result.nextC.length &&
          prev.every((v, i) => v === result.nextC[i])
        ) {
          return prev
        }
        return result.nextC
    ...
    const handleWorkerMessage = async (e: MessageEvent) => {
      ...
      if (type === 'result') {
        ...
        setTimeout(() => {
          ...
          if (isValidAiMove) {
            const update = handleMoveInternal(
              gx,
              gy,
              current.board,
              current.activeConstraint,
  ...
  const handleMove = (gridX: number, gridY: number) => {
    ...
    const result = handleMoveInternal(
      gridX,
      gridY,
      board,
      activeConstraint,
